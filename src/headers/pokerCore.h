...
#include <algorithm>
#include <iterator>
// pokerCore is dependent on
// util.h.
#include "util.h"

/**
 * pokerCore Class (core of program).
 *
 ...
 * instantiate with pokerCore <nameOfObj>(vector <string> pokerHands).
...
class pokerCore {
    private:
     ...
     // Stores the valid card values that a poker hand may have.
     // Legal poker values without wild cards (i.e., no jokers).
     // Note that value card '10' is 'T'.
     //
    ...
    public:
     // Instantiates a pokerCore object for use, it
     ...
     //
     pokerCore::pokerCore(std::vector <std::string> inputHands);
     ...
     // Pops next two pairs to compare from pokerCore object's allHands vector.
	 // As it pops two handsfrom object's allHands vector,  vector's
	 // size will be decremented by 2 every function call.
     // 
     ...
     // of values existing in hand and value occurences.
     ...
     // to get the correct rank value from the pokerCore's rank map.
     //
     ...
     // correct value to the hand given the modulo score obtained. This uses the pokerCore's
     // rank map to do so.
     // 
	 ...
	 // Returns the winner of pair of hands based off results from the compareHands
     // function as an integer. 1 - "hand one won',  2 - "hand two won', 3 - "tie result".
     // 
     // Input is an integer determined in compareHands.
     //      
	 int returnWinner(int& result);
     ...
     // Returns a card. This card is given a value, suit, and magnitude (integer value of the
	 // the card).
     // Example: (if the card's value was 'A' the card's magnitude would be 14).
     //
     // Input is a string that is in the form of "AS".
     //
     card getCard(std::string &input);
     // Returns a string given a card. Where its value, suit, and magnitude are
     // converted back into string form.
     // Example: card 'TC'
     //     its value = 'T', suit = 'C', and magnitude = 10, then "TC" is returned.
     //
     // Input is a card.
     //
     ...
     // Returns an integer specifying how many times a value occurs
     ...
     // Returns a boolean if the specified magnitude cards occurs in the hand.
     //
     // Input is a vector of strings (the hand) and a char specifying what magnitude
     // to search for.
     //
     ...
     // holds what magnitude values occur, and the second bitset holds how many of each magnitude
     ...
     // (value occurences in hand) is used. Searches for a pattern of
     ...
     // Returns an integer from applying the algorithm's deep evaluation 
	 // method. Uses bitwise operations to give a definitive score to a 
	 // hand. Only called if two hands have the same rank.
     //
     // Input is a card vector to determine a definitive hand score based
     // on all cards.
     //
     int getDeepEvaluationScore(std::vector <card> &input);
     // Returns an integer corresponding to a rank in pokerCore's rank map.
